...
class VoiceService:
    ...
    async def evaluate_teach_back(self, topic: str, transcript: str) -> Dict:
        return await chat_agent.evaluate_teach_back(topic, transcript)

    async def generate_study_summary(self, transcripts: List[str], topic: str) -> str:
        combined = "\n".join(transcripts)
...
3. Gives 3-5 actionable study recommendations"""
        return await rag_service.generate_from_prompt(prompt)
