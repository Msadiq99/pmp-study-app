...
export const quiz = {
  generate: (data: { title: string; knowledge_area?: string; num_questions?: number; question_types?: string[] }) =>
...
export const study = {
  startSession: (type?: string, area?: string) =>
    api.post(`/api/study/session?session_type=${type || 'study'}&knowledge_area=${area || ''}`),
...
export const chat = {
  ...
  explain: (concept: string) => api.post(`/api/chat/explain?concept=${concept}`),
};

export const formulas = {
  list: (category?: string) => api.get(`/api/formulas/${category ? `?category=${category}` : ''}`),
  categories: () => api.get('/api/formulas/categories'),
  calculate: (name: string, inputs: any) => api.post(`/api/formulas/calculate?name=${name}`, inputs),
};

export const groups = {
  create: (name: string, description?: string) =>
    api.post(`/api/groups/create?name=${name}&description=${description || ''}`),
